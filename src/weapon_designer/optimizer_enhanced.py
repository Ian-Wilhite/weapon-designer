...
from .fea_viz import render_fea_frame, export_gif
...
def _profile_objective_bspline(x: np.ndarray, cfg: WeaponConfig) -> float:
    ...
    try:
        ...
        bite_score    = min(metrics["bite_mm"] / max_bite, 1.0)
        ...
        score = (
            ...
            + w.mass_utilization     * mass_score * 0.5
...
def _cutout_objective_bspline_polar(
    ...
    try:
        ...
        # Soft mass penalty — gradient-preserving, rewards approaching budget
        mu = metrics["mass_utilization"]
        if mu <= 1.0:
            f_mass = mu                         # reward using more material
        elif mu <= 2.0:
            f_mass = max(0.0, 2.0 - mu)        # linear decay above budget
        else:
            f_mass = 0.0
...
def _cutout_objective_polar(
    ...
    try:
        ...
        # Soft mass factor — rewards approaching budget, gradient-preserving
        mu = metrics["mass_utilization"]
...
def _profile_objective_enhanced(x: np.ndarray, cfg: WeaponConfig) -> float:
    ...
    try:
        ...
        bite_score    = min(metrics["bite_mm"] / max_bite, 1.0)
        balance_score = max(0.0, 1.0 - metrics["com_offset_mm"] / max(max_r * 0.1, 1.0))
        struct_score  = metrics["structural_integrity"]     # FEA-based
        iz_score      = metrics.get("impact_zone", 0.0)
...
class _FEACallback:
    ...
    def __init__(
        ...
        self._log           = log_fn
        self.fea_interval   = max(1, fea_interval)
        self.polar          = polar
    ...
    def __call__(self, xk: np.ndarray, convergence: float) -> bool | None:
        ...
        try:
            ...
            metrics = compute_metrics_enhanced(weapon, self.cfg)
            score   = weighted_score_enhanced(metrics, self.cfg)
            ...
            # ── FEA frame export ─────────────────────────────────────────
            if self.fea_interval > 0 and (self.step % self.fea_interval == 0
                                           or self.step <= 2):
                try:
                    spacing = self.cfg.optimization.fea_fine_spacing_mm
                    fea_data = fea_stress_analysis_with_mesh(
                        weapon,
                        rpm=self.cfg.rpm,
                        density_kg_m3=self.cfg.material.density_kg_m3,
                        thickness_mm=self.cfg.sheet_thickness_mm,
...
def optimize_enhanced(
    ...
    cutout_type  = getattr(cfg.optimization, "cutout_type", "fourier")
    ...
    # Save a final high-quality frame
    ...
    if gif_p1:
        _log(f"GIF saved: {gif_p1}")
    if gif_p2:
        _log(f"GIF saved: {gif_p2}")
